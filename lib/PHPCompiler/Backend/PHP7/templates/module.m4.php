...
if test "$PHP_<?php echo $uppername; ?>" != "no" then
	AS_MSG([Checking for supported PHP versions])
	...
	if test "$PHP_<?php echo $uppername; ?>_FOUND_VERNUM" -lt "700000"; then
		AC_MSG_ERROR([not supported. Need a PHP version >= 7.0.0 (found $PHP_<?php echo $uppername; ?>_FOUND_VERSION)])
	else
